		...
		getVideo: function() {
			...
			if(this.html5Video) { //如果支持HTML5-VIDEO则默认使用HTML5-VIDEO播放器
				...
				if(this.isM3u8) {
					...
					this.loadJs('https://cdn.jsdelivr.net/npm/hls.js@latest', loadJsHandler);
				}
		...
		prompt: function() {
			...
			var showPrompt = function(event) {
				if(thisTemp.promptElement == null) {
					...
					//var random3 = 'prompte' + thisTemp.randomString(5);
					var ele2 = document.createElement('img');
					var ele3 = document.createElement('div');
					ele2.className = 'promptImg';
					ele3.className = 'promptWords';
					if(!thisTemp.PD.childNodes[13]){thisTemp.PD.appendChild(ele2)};
					//thisTemp.PD.appendChild(ele2);
					//thisTemp.PD.appendChild(ele3);
					if(!thisTemp.PD.childNodes[14]){thisTemp.PD.appendChild(ele3)};
					thisTemp.promptElementImg = thisTemp.getByElement('promptImg');
					thisTemp.promptElementWords = thisTemp.getByElement('promptWords');
					thisTemp.css(thisTemp.promptElementImg, {
						overflowX: 'hidden',
						lineHeight: '22px',
						position: 'absolute',
						display: 'block',
					...
					thisTemp.css(thisTemp.promptElementWords, {
						overflowX: 'hidden',
						lineHeight: '22px',
						fontSize: '16px',
						color: '#fff',
				...
				console.log(thisTemp.promptElementImg);
				console.log(thisTemp.PD.lastChild);
				console.log(thisTemp.PD.childNodes);
				thisTemp.PD.childNodes[14].innerHTML = thisTemp.getDataset(this, 'words');
				thisTemp.promptElementImg.setAttribute('src',thisTemp.getDataset(this, 'imgs'));
				thisTemp.css(thisTemp.promptElementImg, {
					left: pL + 'px',
					top: (pT - thisTemp.promptElementImg.offsetHeight - 10) + 'px'
				});
				thisTemp.css(thisTemp.promptElementWords, {
					left: pL + 'px',
					top: (pT - thisTemp.promptElementWords.offsetHeight - 10) + 'px'
				});
			};
			// var showPrompt = function(event) {
			// 	if(thisTemp.promptElement == null) {
			...
			var hidePrompt = function(event) {
				if(thisTemp.promptElementImg != null && thisTemp.promptElementWords !=null) {
					thisTemp.css(thisTemp.promptElementImg, {
						display:"none"
					});
					thisTemp.css(thisTemp.promptElementWords, {
			...
			for(i = 0; i < prompt.length; i++) {
				...
				div.setAttribute('data-imgs', imgs);
				...
				//this.addListenerInside('mouseover', showPromptImg, div);
